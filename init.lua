...
vim.fn.setenv('FZF_DEFAULT_OPTS',
	"--preview-window 'right:50%' --margin=1,4 --bind=alt-k:preview-page-up,alt-j:preview-page-down,ctrl-a:select-all")
...
vim.api.nvim_create_autocmd({ "BufRead", "BufNewFile" },
	{ pattern = { "*.cls", "*.trigger", "*.apex" }, group = ft_group, command = "set filetype=apex syntax=apex" })
...
vim.api.nvim_create_autocmd({ "BufRead", "BufNewFile" },
	{ pattern = "*.soql", group = ft_group, command = "set filetype=apex syntax=sql" })
vim.api.nvim_create_autocmd({ "BufRead", "BufNewFile" },
	{ pattern = "project-scratch-def.json", group = ft_group, command = "set filetype=scratch syntax=json" })
vim.api.nvim_create_autocmd({ "BufRead", "BufNewFile" },
	{
		pattern = { "*.vue", "*.svelte", "*.jsw", "*.cmp", "*.page", "*.component" },
		group = ft_group,
		command =
		"set filetype=html"
	})
vim.api.nvim_create_autocmd({ "BufRead", "BufNewFile" },
	{ pattern = { "*.jsw" }, group = ft_group, command = "set filetype=javascript" })
vim.api.nvim_create_autocmd({ "BufRead", "BufNewFile" },
	{ pattern = "*.jsx", group = ft_group, command = "set filetype=javascript.jsx" })
...
vim.api.nvim_create_autocmd({ "BufRead", "BufNewFile" },
	{ pattern = "**/lwc/*.js", group = ft_group, command = "set filetype=lwc syntax=javascript" })
vim.api.nvim_create_autocmd({ "BufRead", "BufNewFile" },
	{ pattern = "*.rc", group = ft_group, command = "set filetype=sh" })
...
vim.api.nvim_create_autocmd("FileType", {
	...
	group = ft_group,
	callback = function()
		vim.keymap.set("n", "<Leader>$", "<Cmd>%!fxparser | jq<CR><Cmd>set ft=json<CR>ggj4dd<CR>",
...
vim.api.nvim_create_autocmd("FileType", {
	...
	group = ft_group,
	callback = function()
		vim.keymap.set("n", "<Leader>#", "<Cmd>ea 1f<CR><Cmd>set ft=xml<CR>",
			{ buffer = true, noremap = true, silent = true })
	end
})
...
map("n", "<leader>nk", "<Cmd>tabnew ~/.config/kitty/kitty.conf<CR>", opts) -- Edit init.lua
...
map("n", "]tv", "<Cmd>tabnew /tmp/coverage<CR>", opts)
...
map("n", "<leader>[[", "<Cmd>RunAsync sfdx project:retrieve:start -d %<CR>",
	extendOps({ desc = "Refresh current file from default org" }))
...
map("n", "]dd", "<Cmd>RunAsync sfdx project:deploy:start -d % -l NoTestRun -w 5<CR>",
	extendOps({ desc = "Deploy current file to default org" }))
map("n", "]df", "<Cmd>RunAsync sfdx project:deploy:start -d % -l NoTestRun -w 5 -c<CR>",
	extendOps({ desc = "Deploy current file to default org (ignore conflicts)" }))
...
map("n", "]l",
	":<C-u>tabnew /tmp/apexlogs.log<CR><C-w>s<C-w>j:term sfdx apex:tail:log --color -o | tee /tmp/apexlogs.log<C-left><C-left><C-left>",
	opts)
map("n", "]ll",
	"<Cmd>tabnew /tmp/apexlogs.log<CR><C-w>s<C-w>j:term sfdx apex:tail:log --color | tee /tmp/apexlogs.log<CR>", opts)
...
map("n", "]gl", function() -- Get log by ID under cursor
	...
	vim.cmd("normal! 0")
	vim.cmd("normal! /\\%" .. current_line .. "l07L")
	vim.cmd("nohlsearch")
...
map("i", "<c-x><c-k>", "fzf#vim#complete#word({'window': { 'width': 0.2, 'height': 0.9, 'xoffset': 1 }})",
...
map("i", "<C-x>m", [[fzf#vim#complete({'source': 'cat schema.txt', 'reducer': { lines -> split(lines[0],' ')[0]}})]],
	{ expr = true, noremap = true })
...
-- Git Log Graph
map("n", "<leader>gr",
	"<Cmd>tabnew | startinsert | term git log --all --decorate --graph --color=always<CR>",
	opts)
map("n", "<leader>gj", "<Cmd>tabnew | startinsert | term jj log --color=always<CR>", opts)
...
-- vim.g.copilot_settings = { selectedCompletionModel = "gpt-4o-copilot" }  -- Replace with desired model like "claude-3-5-sonnet" or others available to your account
...
require 'luasnips_extras'            -- Additional luasnips configurations

require 'ast_grep_ls'


require 'agent_script'
